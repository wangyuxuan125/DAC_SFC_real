...
void LinearControl::DARE(const Eigen::MatrixXd &A, const Eigen::MatrixXd &B, const Eigen::MatrixXd &Q, const Eigen::MatrixXd &R,
  ...
  while (true) {
    ...
    // compute new P
    P = Acal_T * P * Acal -
        Acal_T * P * B * (R + B_T * P * B).inverse() * B_T * P * Acal + Qcal;//最小代价函数
...
LinearControl::DLQR_Control(const Desired_State_t &des,
    ...
    double sin = std::sin(des.yaw);
    double cos = std::cos(des.yaw);
...
LinearControl::calculateControl(const Desired_State_t &des,
    ...
    * Eigen::AngleAxisd(roll,Eigen::Vector3d::UnitX());
    ...
    std::ofstream log_filet("/home/uav/data_D_DLQR/time.txt", std::ios::app); // 以追加模式打开文件
    log_filet << "time:" << time_1<<std::endl;
    log_filet.close();   

    std::ofstream log_filex("/home/uav/data_D_DLQR/xv.txt", std::ios::app); // 以追加模式打开文件
    log_filex << "x:" << des.v(0)<<std::endl;
    log_filex.close();

    std::ofstream log_filey("/home/uav/data_D_DLQR/yv.txt", std::ios::app); // 以追加模式打开文件
    log_filey << "y:" << des.v(1)<<std::endl;
    log_filey.close();

    std::ofstream log_filez("/home/uav/data_D_DLQR/zv.txt", std::ios::app); // 以追加模式打开文件
    log_filez << "z:" << des.v(2)<<std::endl;
    log_filez.close();
...
bool LinearControl::estimateThrustModel(
    const Eigen::Vector3d &est_a,const Parameter_t &param) {
