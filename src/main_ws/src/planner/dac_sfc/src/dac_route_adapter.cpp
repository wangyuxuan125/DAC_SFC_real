                            ...
                            RouteDiagnostics &diagnostics)
{
  ...
  {
    const ASTAR_RET search_result = astar_->AstarSearch(map_->getResolution(), start, goal);
  ...
  if (diagnostics.direct_path)
  {
    const int segment_count = std::max(
        1, static_cast<int>(std::ceil(
               (goal - start).norm() / options.max_segment_length)));
    for (int i = 1; i <= segment_count; ++i)
    {
      const double alpha = static_cast<double>(i) /
                           static_cast<double>(segment_count);
      sparse_route.push_back((1.0 - alpha) * start + alpha * goal);
    }
