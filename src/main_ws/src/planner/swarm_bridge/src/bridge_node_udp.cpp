...
// int udp_server_fd_, udp_send_fd_;
// ros::Subscriber other_odoms_sub_, one_traj_sub_, mandatory_stop_sub_, goal_sub_, joy_sub_, my_map_sub_;
// ros::Publisher other_odoms_pub_, one_traj_pub_, mandatory_stop_pub_, goal_pub_, joy_pub_, others_map_pub_;
...
// sensor_msgs::PointCloud2 others_map_cloud_msg_; 
...
// void map_sub_udp_cb(const sensor_msgs::PointCloud2ConstPtr &msg)
// {
//   static ros::Time t_last;
//   ros::Time t_now = ros::Time::now();

//   if (map_broadcast_freq_ > 0.0 &&
//       (t_now - t_last).toSec() * map_broadcast_freq_ < 1.0) // 限频：map_broadcast_freq_
//   {
//     return;
//   }
//   t_last = t_now;
...
//   msg_out.header.frame_id = std::string("drone_") + std::to_string(drone_id_) + "_map";
//   // msg_out.header.frame_id = "world";

//   int len = serializeTopic(MESSAGE_TYPE::MAP_CLOUD, msg_out);

//   if (sendto(udp_send_fd_, udp_send_buf_, len, 0,
//              (struct sockaddr *)&addr_udp_send_, sizeof(addr_udp_send_)) <= 0)
//   {
//     ROS_ERROR("UDP SEND ERROR (MAP_CLOUD)!!!");
//   }
// }
...
void udp_recv_fun()
{
  ...
  while (ros::ok())
  {
    ...
    // case MESSAGE_TYPE::MAP_CLOUD:
    // {
    //   if (valread == deserializeTopic(others_map_cloud_msg_))
    //   {
    //     others_map_pub_.publish(others_map_cloud_msg_);
    //   }
    //   else
    //   {
    //     ROS_ERROR("Received message length not matches the sent one (MAP_CLOUD)!!!");
    //     continue;
    //   }
...
int main(int argc, char **argv)
{
  ...
  // nh.param("map_max_freq",  map_broadcast_freq_,  1.0); 
  ...
  // my_map_sub_ = nh.subscribe("my_map_cloud", 1, map_sub_udp_cb, ros::TransportHints().tcpNoDelay());
  // others_map_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/others_map_cloud", 10);
